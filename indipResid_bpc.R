indipResid_bpc <- function(data,        # the data
                           G,           # adj matrix
...
){       # which test to do in hsic (1 = permut, 2 = gamma)
  ...
  dep1 = IndepTestBr(x = res1, y = data[, S], p = p)        #  HSIC(X = res1, Y = data[, S], sig = sig, p = p, test = test)
  ...
  for( i in 1:length(S) ) # for every node to which v is undirectly connected
  {
    ...
    dep2 = IndepTestBr(x = res2, y = data[, v ], p = p)    # HSIC(X = res2, Y = data[, v ], sig = sig, p = p, test = test)
    ...
    if(length(Sj)>0)
    {
      ...
      {        
        ...
        for( k in 1:ncol(sub))
        {
          ...
          dep2 = IndepTestBr(x = res2, y = data[, c(v, sub[,k]) ], p = p)            #HSIC(X = res2, Y = data[, c(v, sub[,k]) ], sig = sig, p = p, test = test)
          print(paste0('The second is',dep2,'   I return 0 if dep2>alpha'))
