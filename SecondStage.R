            ...
            if( t <= length(vectUndEdgeUi) )
            {
              ...
              {     
                ...
                if( NOTImmort(G, nodeS[i], sub[,j]) == 1)
                {
                  ...
                  ir = indipResid_bpc(data = data, G = G, undiEdge = undiEdge, v = nodeS[i], parent = parent, S = sub[,j], alpha =  alpha, p = p) ## risposta binaria 0 o 1
                  print(paste0('==> ir=',ir))
                  if( ir == 1)
                  {
                    print('Enter')
                    ...
                    G[sub[,j], nodeS[i]] = 1   # I do the orientation I just found
                    G[nodeS[i], sub[,j]] = 0
                    ...
                    { 
                      ...
                      {
                        print('I entered in FINAL DIRECTION:')
                    ...
                    t = 1  # ISN'T IT UNUSEFULL?
                    
                    K = SecondStage(data = data, G_ad = G, G_dir = G_dir, undiEdge = undiEdge, s, alpha = alpha, p = p) ## AM I SURE ABOUT THIS???
                    return(K)
